...
export interface AssistantTurn {
  ...
  expects:
    ...
    | { kind: 'product'; products: Array<{ id: string; title: string; imageUrl: string; priceCents: number }> }
    | { kind: 'confirm' }
}
...
function applyAnswer(
  ...
  switch (turn.stage) {
    case 'choosing_path':
      if (answer.kind === 'choice' && (answer.choice === 'HAS_STORE' || answer.choice === 'HAS_SOURCE_LINK' || answer.choice === 'DESCRIBE_ONLY')) {
...
function scriptedTurn(ctx: ConversationContext): AssistantTurn {
  const info = ctx.collectedInfo
  if (!info.inputPath) {
    return {
      stage: 'choosing_path',
      message: '欢迎来到 Forgely。我们先确认输入：你属于下面哪种情况？',
      reasoning: '把用户分流到三条路径，决定后续问题策略。',
      expects: {
        kind: 'choice',
        options: [
  ...
  if (info.inputPath === 'DESCRIBE_ONLY' && !info.brandDescription) {
    return {
      ...
      message: '一句话告诉我你的品牌：你卖什么、给谁、想传达什么感觉？',
      reasoning: '没有源数据，用文字提取品牌主意。',
      expects: { kind: 'text', multiline: true, placeholder: '例如：手工木玩具，给追求慢生活的欧美父母…' },
    }
  }
  if (!info.targetAudience) {
    return {
      stage: 'gathering_audience',
      message: '一句话描述你最想触达的海外消费者画像。',
      reasoning: '客群是 Visual DNA 推荐和文案 tone 的关键。',
      expects: { kind: 'text', placeholder: '例如：30-45 岁北美中产新手父母，注重设计与可持续' },
    }
  }
  if (!info.referenceBrands || info.referenceBrands.length === 0) {
    return {
      stage: 'gathering_references',
      message: '列出 3 个你欣赏的海外品牌或参考站点（直接打名字即可）。',
      reasoning: 'Reference brands 是 archetype 匹配的捷径。',
      expects: {
        kind: 'tags',
        suggestions: ['Aesop', 'Grimm’s', 'Recess', 'BIOLOGICA', 'Liquid Death', 'HAY', 'Jacquemus', 'Apple'],
      },
    }
  }
  if (!info.moodKeywords || info.moodKeywords.length === 0) {
    return {
      ...
      message: '选 3-5 个最贴近你想要的视觉调性的关键词。',
      reasoning: '调性关键词决定 Visual DNA 推荐排序。',
      expects: {
        kind: 'tags',
        suggestions: ['warm', 'minimal', 'cinematic', 'bold', 'playful', 'organic', 'editorial', 'futuristic'],
      },
    }
  }
  if (!info.selectedDnaId) {
    return {
      stage: 'choosing_dna',
      message: '基于你提供的信息，我推荐这 3 个 Visual DNA — 选一个最贴你想象的。',
      reasoning: '把推荐落地为具体可视决策。',
      expects: {
        kind: 'choice',
        options: [
  ...
  if (!info.format) {
    return {
      ...
      message: '首屏 Hero 你想要视频还是 3D 互动？',
      reasoning: '3D 仅 Pro+ 可解锁；视频默认。',
      expects: {
        kind: 'choice',
        options: [
  ...
  if (!info.selectedMomentId) {
    return {
      stage: 'choosing_hero',
      message: '请选一个 Hero Moment（默认推 M04 Breathing Still — 最稳）。',
      reasoning: '让用户对叙事节奏有掌控感。',
      expects: {
        kind: 'choice',
        options: [
  ...
  return {
    ...
    reasoning: '所有必填字段齐了，进入最终确认。',
    expects: { kind: 'confirm' },
  }
}

/** Helper for the worker layer — turn a finished context into the runPipeline input. */
export function toPipelineInput(ctx: ConversationContext, opts: { siteId: string; subdomain: string; brandName: string }): {
