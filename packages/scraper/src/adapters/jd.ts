...
/**
 * JD.com (京东) item page adapter. Most JD product URLs look like
 * `https://item.jd.com/100012345678.html`. Price is loaded via XHR on PC,
 * but the rendered (`render=true`) page exposes both `.p-price` markup
 * and a server-side `og:price:amount` meta when caching is fresh.
 */
...
export class JdAdapter implements ScraperAdapter {
  ...
  async scrape(url: string, options: ScrapeOptions = {}): Promise<ScrapedData> {
    ...
    const vendor = $('.J-hove-wrap .name a, .seller-infor a').first().text().trim() || undefined
...
function collectImages($: cheerio.CheerioAPI, base: string): ScrapedImage[] {
  ...
  $('#spec-list img, .preview-pic img, .lh img').each((_, el) => {
