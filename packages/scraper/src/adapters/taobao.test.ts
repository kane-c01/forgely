...
import { taobaoItemHtml, tmallItemHtml } from '../__tests__/fixtures/china.js'
...
describe('TaobaoAdapter.scrape', () => {
  it('parses a Taobao item page (CNY)', async () => {
    const adapter = new TaobaoAdapter({ scraperApi: client(taobaoItemHtml) })
  ...
  it('marks Tmall pages with platform=tmall', async () => {
    const adapter = new TaobaoAdapter({ scraperApi: client(tmallItemHtml) })
