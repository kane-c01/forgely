...
/**
 * Subset of `playwright` we depend on. Tests can inject a fake matching this
 * shape without bundling the real browser.
 */
 ...
 * contract. Lives in its own package so the core scraper stays Worker/Lambda-friendly.
 */
  ...
  async screenshot(url: string, options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    ...
    try {
      ...
      try {
        ...
        // Uint8Array but introduces extra `type: 'Buffer'` baggage that
        // breaks deep equality with downstream code).
