...
export default function DashboardPage() {
  ...
  // tRPC — auth.me powers the greeting; sites.list pins the "primary" site
  // at the top of the dashboard. Both fall back to mocks when DB is empty.
  const me = trpc.auth.me.useQuery(undefined, { retry: false })
  const sitesQuery = trpc.sites.list.useQuery({}, { retry: false })
  const { greeting, time, date } = useGreeting((me.data?.name?.split(' ')[0] ?? 'Alex') as string)
  const copilot = useCopilot()
  // Pick the first live site if available; otherwise show the canned demo
  // so the page renders even without data.
  const liveSites = Array.isArray(sitesQuery.data)
    ? (sitesQuery.data as Array<Record<string, unknown>>)
    : ((sitesQuery.data as { rows?: Array<Record<string, unknown>> } | undefined)?.rows ?? [])
  const primary = liveSites[0]
  const isDemo = !primary
  const site = primary
    ? {
        id: String(primary.id ?? defaultSite.id),
        name: String(primary.name ?? defaultSite.name),
        domain: String(primary.subdomain ?? primary.domain ?? defaultSite.domain),
        metrics: defaultSite.metrics,
      }
    : defaultSite
      ...
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        {time && date ? (
          <div className="text-caption text-text-muted flex items-center gap-3 font-mono uppercase tracking-[0.18em]">
            ...
            <Badge tone={isDemo ? 'outline' : 'success'} dot={!isDemo}>
