...
### 架构决策更新（2026-04-19 W5 会话）

| 决策                             | 详情                                                                                          |
| -------------------------------- | --------------------------------------------------------------------------------------------- |
| **超管后台不强制境内部署**       | `/super` 路由走 Cloudflare / Vercel 全球节点，不需要 ICP 备案                                 |
| **站点托管区域选择（付费增值）** | 用户可额外 ¥49/月选 US / EU / APAC 专属区域（Workers 计算 + R2 存储），默认 `auto`（免费）    |
| **自定义域名关联**               | 用户自行注册域名 → Forgely 后台生成 CNAME 指引 → 自动 DNS 验证 + Cloudflare for SaaS SSL 签发 |
| **超管可直接关联域名**           | `/super/sites` 面板支持超管为用户跳过自助流程直接绑定域名                                     |

对应文件变更：

- `docs/PIVOT-CN.md` §6.1–§6.4（文档）
- `services/api/prisma/schema.prisma`：`Site.hostingRegion` + `SiteCustomDomain` + `SiteHostingAddon` 三个新模型
- `services/deploy/src/index.ts`：区域感知部署 + Cloudflare for SaaS 域名管理 API
- `apps/app/lib/super/mock-extras.ts` + `apps/app/app/super/sites/_components/SitesClient.tsx`：超管面板展示
